...
<div class="max-w-screen-xl mx-auto bg-[#121212] p-10 rounded-2xl shadow-xl border border-gray-800">
    <h1 class="text-3xl font-extrabold mb-8 text-[#f37021] tracking-wide">EDIT SERVICE</h1>
    ...
    <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div>
            ...
            <textarea name="short_description" id="short_description" rows="3"
                class="w-full bg-[#1a1a1a] text-white placeholder-gray-400 border border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#f37021] focus:border-[#f37021] transition"
                placeholder="Enter short summary">{{ old('short_description', $service->short_description) }}</textarea>
        ...
        <div>
            ...
            <textarea name="overview" id="overview" rows="5"
                class="w-full bg-[#1a1a1a] text-white border border-gray-700 rounded-lg px-4 py-2.5">{{ old('overview', $service->overview) }}</textarea>
            @error('overview')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="offer" id="offer" rows="5"
                class="w-full bg-[#1a1a1a] text-white border border-gray-700 rounded-lg px-4 py-2.5">{{ old('offer', $service->offer) }}</textarea>
            @error('offer')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="border-2 border-dashed border-gray-600 rounded-xl p-8 text-center bg-[#1a1a1a] hover:border-[#f37021]/60 transition">
            ...
            <p class="text-sm text-gray-400 mb-5">Upload a new icon to replace the existing one</p>
            ...
            @if ($service->icon)
            <div id="iconPreviewContainer" class="mt-6 text-center">
                <p class="font-semibold text-gray-300 mb-3">Current Icon</p>
                <div id="iconPreview" class="relative inline-block">
                    <img src="{{ asset($service->icon) }}" class="w-32 h-32 object-contain rounded-lg border border-gray-700 shadow-md" alt="Service Icon">
                </div>
            </div>
            @else
        ...
        <div class="border-2 border-dashed border-gray-600 rounded-xl p-8 text-center bg-[#1a1a1a] hover:border-[#f37021]/60 transition">
            ...
            <p class="text-sm text-gray-400 mb-5">Upload a new thumbnail to replace the existing one</p>
            ...
            <div id="thumbnailPreviewContainer" class="mt-6 text-center">
                <p class="font-semibold text-gray-300 mb-3">Current Thumbnail</p>
                <div id="thumbnailPreview" class="relative inline-block">
                    <img src="{{ asset($service->thumbnail) }}" class="w-80 h-48 object-cover rounded-lg shadow-lg border border-gray-700" alt="Service Thumbnail">
                </div>
            </div>
            @else
        ...
        <div class="border-2 border-dashed border-gray-600 rounded-xl p-8 text-center bg-[#1a1a1a] hover:border-[#f37021]/60 transition">
            ...
            <div class="mt-6 text-center">
                ...
                <div class="flex flex-wrap justify-center gap-3">
                    ...
                    <img src="{{ asset($image->image) }}" class="w-24 h-24 object-cover rounded-lg border border-gray-700 shadow-md" alt="Gallery Image">
                    @endforeach
                </div>
            </div>
            @endif
...
<script>
    ...
    function setupSinglePreview(inputId, containerId, previewId) {
