...
@section('content')
<!-- Page Title -->
<h1 class="text-2xl font-semibold mb-6">PROJECT CATEGORIES</h1>
...
<div class="overflow-x-auto bg-white border rounded-lg">
    <table class="table-fixed w-full border-collapse" id="project-categories-table">
        ...
        <tbody>
            ...
            <tr class="border-t" id="row-{{ $projectCategory->id }}">
                <td class="px-10 py-3 text-center">{{ $projectCategory->id }}</td>
                <td class="px-10 py-3 text-center">{{ $projectCategory->category_name }}</td>
                <td class="px-6 py-3">
                    <div class="flex justify-center items-center gap-2">
                        <button type="button"
                            onclick="deleteCategory({{ $projectCategory->id }})"
...
<div id="create-modal"
    ...
    <div class="bg-white rounded-3xl shadow-lg p-8 w-[600px] max-w-full animate-fade-in">
        ...
        <form id="create-form" method="POST" action="{{ route('admin.projectCategories.store') }}">
            @csrf
            <div class="mb-4">
...
<script>
    ...
    function deleteCategory(categoryId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/admin/projectCategories/" + categoryId,
                    method: "POST",
                    data: {
                        _method: "DELETE"
                    },
    ...
    $(document).ready(function() {
        ...
        $('#create-form').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('admin.projectCategories.store') }}",
                method: "POST",
                data: $(this).serialize(),
                success: function(response) {
                    table.row.add([
